...
def run_code_submission(submission_id) -> bool:
    ...
    try:
        ...
        logger.error(
            f"[submission_run][id={submission_id}] Submission Not Found"
        )
        return False
    ...
    try:
        ...
        e = sys.exc_info()[0]
        logger.error(
            f"[submission_run][id={submission_id}] Camisole request error : {e}"
        )
        return False
    ...
    try:
        passed = test_passes(
            camisole_response["tests"], submission.problem.tests
        )
    except:
        e = sys.exc_info()[0]
        logger.error(
            f"[submission_run][id={submission_id}] Camisole response malformed: {e}"
        )
        return False
